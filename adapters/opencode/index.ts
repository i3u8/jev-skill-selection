/**
 * OpenCode plugin — hard filter by default via tool.definition;
 * soft inject via chat.message only when JEV_FILTER_MODE=soft|both.
 *
...
export type FilterMode = "hard" | "soft" | "both";

function env(name: string, fallback = ""): string {
  return (process.env[name] ?? fallback).trim();
}

export function filterModeFromEnv(defaultMode: FilterMode = "hard"): FilterMode {
  const raw = (env("JEV_FILTER_MODE", defaultMode) || defaultMode).toLowerCase();
  if (raw === "hard" || raw === "soft" || raw === "both") return raw;
  return defaultMode;
}

export function wantsHard(mode: FilterMode = filterModeFromEnv()): boolean {
  return mode === "hard" || mode === "both";
}

export function wantsSoft(mode: FilterMode = filterModeFromEnv()): boolean {
  return mode === "soft" || mode === "both";
}
...
export function filterAvailableSkills(params: any, kept: Set<string>): any {
...
function applyHardToolFilter(output: any, lastKept: Set<string>, lastDropped: Set<string>): void {
  if (!output || lastKept.size === 0) return;
...
function buildHooks() {
  ...
  let lastSoft = "";
  let lastFilterMode: FilterMode = filterModeFromEnv();
  ...
  return {
    "chat.message": async (input: any, output: any) => {
      ...
      lastFilterMode = filterModeFromEnv();
      const soft = wantsSoft(lastFilterMode);
      const hard = wantsHard(lastFilterMode);
      const ctx = soft ? formatSoftContext(result) : "";
      lastSoft = ctx;
      e2eLog(message, result, ctx, lastFilterMode, hard);

      if (!soft || !output || typeof output !== "object") return;
    ...
    "experimental.chat.system.transform": async (_input: any, output: any) => {
      if (!wantsSoft(lastFilterMode) || !lastSoft || !output || !Array.isArray(output.system)) return;
      output.system.push(lastSoft);
    },
    "tool.definition": async (input: any, output: any) => {
      if (!wantsHard(lastFilterMode)) return;
      ...
      applyHardToolFilter(output, lastKept, lastDropped);
    },
  };
}
